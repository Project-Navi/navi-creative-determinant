...
Core definitions---the semiotic manifold, viability closure, PDE operators, and boundary value problem structure---together with the spectral characterization theorem are machine-verified in Lean~4 against the Mathlib library; numerical validation confirms \(O(h^2)\) convergence across grid resolutions. All proofs, computations, and verification artifacts are open source.
\end{abstract}
...
The mathematical foundations of this framework are machine-verified using the Lean~4 theorem prover with the Mathlib library. The formalization covers the semiotic manifold structure, coefficient bounds, the Laplace--Beltrami and gradient-norm operators, the full boundary value problem~\eqref{eq:mainBVP}, and the definition of weak coherent configurations. The spectral characterization theorem---that \(\beta > (\pi/L)^2/b\) implies \(\lambda_1 < 0\)---and a scaling algebraic lemma used in uniqueness arguments are proved in Lean~4 without \texttt{sorry} axioms. The existence theorems (Theorems~\ref{thm:existenceV1prime} and~\ref{thm:nontrivialV1prime}) are proved in Lean conditional on a \texttt{PdeInfra} typeclass that axiomatizes the classical PDE infrastructure (Schaefer fixed point, Schauder estimates, maximum principle, sub-/super-solution theory); these results are not yet available in Mathlib for abstract Riemannian manifolds. The axiom boundary is explicit: each assumption corresponds to a named classical result, and the \texttt{\#print axioms} command in the verification module confirms that no \texttt{sorry} axioms appear. Numerical validation with \(O(h^2)\) grid convergence supplements the formal proofs. All Lean source files, continuous integration pipelines, and computational artifacts are available at \url{https://github.com/Project-Navi/navi-creative-determinant}.
