...
\title{The Creative Determinant: Autopoietic Closure as a\\
Nonlinear Elliptic Boundary Value Problem\\
with Lean~4-Verified Existence Conditions}
...
\begin{abstract}
We present a formal mathematical realization of autopoietic closure in the sense of Maturana and Varela~\cite{MaturanaVarela1980}, expressed as a nonlinear elliptic boundary value problem on a compact Riemannian manifold.
The \emph{Creative Determinant} (CD) framework asks: \emph{when can coherent presence sustain itself?}
On a compact Riemannian manifold \(M\) (the \emph{semiotic manifold}), we study the BVP with logistic saturation,
\[
-\Delta \Phi = a(x)\,\abs{\nabla \Phi} + b(x)\,\Phi - c(x)\,\Phi^p\quad\text{in }M,\qquad \Phi=0\quad\text{on }\partial M,
\]
where \(a(x)=\kappa(x)\gamma(x)\mu(x)\in[0,1]\) encodes the \emph{creative drive} (care \(\times\) coherence \(\times\) contradiction), \(b(x)\) is the \emph{viability potential} (autopoietic support minus dissolution, with canonical closure \(b=\kappa\gamma - \lambda\mu\)), and \(c(x)\Phi^p\) enforces \emph{carrying capacity}.
The principal eigenvalue condition \(\lambda_1(-\Delta - b; M) < 0\) provides an exact, computable threshold for the emergence of self-sustaining coherent configurations---formalizing, for the first time, the transition from incoherence to autopoietic organization.
We establish existence of \emph{weak coherent configurations} via Schaefer's fixed-point theorem with explicit a priori bounds, and prove a nontriviality theorem: when viability exceeds dissipation, the BVP admits a positive solution and coherent presence can be self-maintained.
Beyond the PDE core, we distinguish weak from strong coherence (a gap we treat as the framework's central generative tension), formulate conditional geometric statements, and propose an operational \emph{Creative Determinant condition} \(CD(\alpha,\varepsilon,\delta)\) with five explicit falsifiability criteria.
Core definitions---the semiotic manifold, viability closure, PDE operators, and boundary value problem structure---together with fifteen supporting theorems spanning spectral characterization, operator algebra, coefficient bounds, scaling uniqueness, and fixed-point existence are machine-verified in Lean~4 against the Mathlib library; numerical validation confirms \(O(h^2)\) convergence across grid resolutions. All proofs, computations, and verification artifacts are open source.
\end{abstract}

\paragraph{Keywords.}
autopoiesis; elliptic PDE; formal verification; Lean~4; fixed-point theorems; enactivism; dynamical systems; AI measurement.
...
\]
The zero-order terms \(b\,u\) and \(c(u_+)^p\) are bounded in \(C^{0,\alpha}\) by a constant depending only on \(\|b\|_{C^{0,\alpha}},\|c\|_{C^{0,\alpha}}\) and \(K\).
For the gradient term, apply the Schauder interior estimates~\cite[Thm.~6.6]{GilbargTrudinger2001}:
\[
\|u\|_{C^{2,\alpha}} \le C_1\Big(\|f_u\|_{C^{0,\alpha}} + \|u\|_{C^0}\Big).
\]
The term \(\|a|\nabla u|\|_{C^{0,\alpha}}\) appearing in \(\|f_u\|_{C^{0,\alpha}}\) is controlled by
\(\|a\|_{C^{0,\alpha}}\|\nabla u\|_{C^{0,\alpha}}\).
Now apply the interpolation inequality~\cite[Lemma~6.32]{GilbargTrudinger2001}: for any \(\varepsilon>0\),
\[
\|\nabla u\|_{C^{0,\alpha}} \le \varepsilon\,\|u\|_{C^{2,\alpha}} + C(\varepsilon)\,\|u\|_{C^0}.
\]
Substituting and choosing \(\varepsilon>0\) small enough that \(C_1\|a\|_{C^{0,\alpha}}\varepsilon < \tfrac{1}{2}\), we absorb the \(\|u\|_{C^{2,\alpha}}\) term from the right-hand side:
\[
\|u\|_{C^{2,\alpha}} \le C_2\Big(1 + \|u\|_{C^0}\Big) \le C_2(1+K).
\]
Consequently \(\|u\|_{C^{1,\alpha}}\le \|u\|_{C^{2,\alpha}}\) is uniformly bounded.
\end{proof}
...
Define \(\overline{\Phi}:=K\phi_1^2\) with \(K>0\) to be chosen.
Since \(\phi_1=0\) on \(\partial M\), we have \(\overline{\Phi}=0\) on \(\partial M\) and \(\overline{\Phi}>0\) in \(M^\circ\).
Computing the Laplacian using the product rule and the eigenequation \eqref{eq:eigRewrite}:
\[
-\Delta(\phi_1^2) = 2|\nabla\phi_1|^2 + 2\phi_1(-\Delta\phi_1) = 2|\nabla\phi_1|^2 + 2(b-\beta)\phi_1^2.
\]
Thus \(-\Delta\overline{\Phi} = K\big(2|\nabla\phi_1|^2 + 2(b-\beta)\phi_1^2\big)\).
The super-solution condition \(-\Delta\overline{\Phi}\ge a|\nabla\overline{\Phi}| + b\overline{\Phi} - c(\overline{\Phi})^p\) reads, after dividing by \(K>0\):
\begin{equation}\label{eq:supersol_ineq}
2|\nabla\phi_1|^2 + 2(b-\beta)\phi_1^2 \ge 2a\phi_1|\nabla\phi_1| + b\phi_1^2 - cK^{p-1}\phi_1^{2p}.
\end{equation}
We verify \eqref{eq:supersol_ineq} in two regions.

\emph{Near the boundary} (\(\phi_1<\delta\)):
The dominant terms on the left are \(2|\nabla\phi_1|^2\), while on the right the leading term is \(2a\phi_1|\nabla\phi_1|\).
By the Hopf lemma, \(|\nabla\phi_1|\ge c_H>0\) in a collar neighborhood of \(\partial M\), while \(\phi_1\to 0\).
Hence for \(\delta\) small enough, \(2|\nabla\phi_1|^2 \ge 2a\phi_1|\nabla\phi_1|\) (since \(a\le 1\) and \(\phi_1<\delta\ll |\nabla\phi_1|\)), and the remaining terms are controlled.

\emph{In the interior} (\(\phi_1\ge\delta\)):
The saturation term \(cK^{p-1}\phi_1^{2p}\ge c_0 K^{p-1}\delta^{2p}\) grows without bound as \(K\to\infty\), while the other terms are \(K\)-independent and bounded on the compact set \(\{\phi_1\ge\delta\}\).
Choose \(K\) large enough that this term absorbs all others.

Combining, \(\overline{\Phi}=K\phi_1^2\) is a super-solution for \(K\) sufficiently large.

\paragraph{Step 3: ordering and conclusion.}
By construction, \(\underline{\Phi}=\varepsilon\phi_1\ge 0\) and \(\overline{\Phi}=K\phi_1^2\ge 0\).
...
Def.\ 3.1 (Weak coherent config.) & \texttt{IsWeakCoherentConfiguration} & \texttt{Basic} & Definition \\
Def.\ 3.13 (Principal eigenvalue) & \texttt{PrincipalEigendata} & \texttt{Axioms} & Definition \\
\midrule
Lemma 3.7 (Compactness of $T$) & \texttt{PDEInfra.T\_compact} & \texttt{Axioms} & Axiom \\
...
Lem.\ 3.10 (algebraic core) & \texttt{rpow\_le\_of\_mul\_rpow\_le} & \texttt{LinftyAlgebraic} & Proved \\
Lem.\ 3.10 (algebraic bound) & \texttt{linfty\_bound\_algebraic} & \texttt{LinftyAlgebraic} & Proved \\
...
\midrule
--- (Viability threshold) & \texttt{viabilityThreshold} & \texttt{Theorems} & Definition \\
--- (Solution operator) & \texttt{SolutionOperator} & \texttt{Axioms} & Definition \\
--- (Abstract operators) & \texttt{SemioticOperators} & \texttt{Basic} & Definition \\
...
--- (Knaster--Tarski) & \texttt{OrderHom.nextFixed\_le\_of\_le} & \texttt{MonotoneFixedPoint} & Proved \\
--- (Monotone fixed point) & \texttt{monotone\_fixed\_point\_between} & \texttt{MonotoneFixedPoint} & Proved \\
\midrule
...
\midrule
\texttt{T\_compact} & Schauder estimates + Arzel\`a--Ascoli & No H\"older spaces on manifolds \\
...
The \texttt{T\_compact} field uses Mathlib's bornological vocabulary: it asserts that $T$ maps von Neumann bounded sets to relatively compact sets (\texttt{IsVonNBounded} $\to$ \texttt{IsCompact}), following the compact-operator characterization suggested on the Lean Zulip. The Schaefer axiom takes this as an explicit hypothesis, making the dependency structurally visible. The algebraic core of the $L^\infty$ bound (Lemma~\ref{lem:LinftyBound})---that $bv \geq cv^p$ implies $v \leq (B/c_0)^{1/(p-1)}$---is proved in the main formalization (\texttt{LinftyAlgebraic.lean}); the maximum-principle step (that $\nabla u = 0$ and $\Delta u \leq 0$ at an interior maximum) remains an axiom.
